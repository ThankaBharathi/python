...
from cortex.monitor.live_monitor_ui import MonitorUI, LiveMonitorUI
...
class CortexCLI:
    ...
    # Collects a one-time snapshot of CPU, RAM, disk, and network usage and prints system health.
    def monitor(self, args: argparse.Namespace) -> int:
    """Show current system resource usage."""
    monitor = ResourceMonitor(interval=1.0)

    try:
        cx_print("📊 System Health:", "info")

        # If duration is specified, monitor for that time
        if getattr(args, "duration", None):
            cx_print(f"Monitoring for {args.duration} seconds...", "info")
            monitor.monitor(duration=args.duration)
            metrics = self._get_latest_metrics(monitor)
        else:
            # Single snapshot
        ...
        # Show performance recommendations if history exists
        if monitor.history and len(monitor.history) > 1:
        ...
        # IMPORTANT: always stop background threads
        monitor.stop()
    ...
    def _export_monitor_data(
        ...
        if output:
            ...
            safe_name = "".join(c if c.isalnum() else "_" for (software or "monitor"))
    ...
    def install(
        ...
        if monitor and not execute:
            cx_print("--monitor enabled execution automatically", "info")
            execute = True
            
        if export and not monitor:
            self._print_error("--export requires --monitor flag to be enabled")
            return 1
        ...
        if monitor:
            ...
            print(MonitorUI.format_installing_header(software))
        
        # Live Monitor UI
        # Live Monitor UI - only start if monitor is enabled and resource_monitor exists
        live_ui = None
        
        if monitor and resource_monitor:
            try:
                live_ui = LiveMonitorUI(resource_monitor, title=f"Installing {software}...")
                live_ui.start()
            except Exception as e:
                # If live UI fails, continue with normal monitoring
                if self.verbose:
                    console.print(f"[dim]Live UI failed to start: {e}[/dim]")
                live_ui = None
        
        def cleanup_live_ui():
            """Clean up the live monitoring UI."""
            if live_ui:
                try:
                    live_ui.stop()
                except Exception as e:
                    if self.verbose:
                        console.print(f"[dim]Error stopping live UI: {e}[/dim]")
        ...
        try:
            ...
            if execute:
                ...
                def progress_callback(current, total, step):
                    ...
                    # Only print if Live UI is not active
                    if resource_monitor and not live_ui:
                        metrics = self._get_latest_metrics(resource_monitor)
                        print(MonitorUI.format_installation_metrics(metrics))
                ...
                if result.success:
                    ...
                    if monitor and resource_monitor:
                        ...
                        print(MonitorUI.format_peak_usage(peak))
                     ...
                     # Clean up live UI
                    if 'cleanup_live_ui' in locals():  # Check if cleanup function exists
                        cleanup_live_ui()
